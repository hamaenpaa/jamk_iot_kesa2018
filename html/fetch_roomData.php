...
if (isset($_GET['rid'])) {
	...
	if ($res_getRL = $conn->prepare("SELECT ca_roomlog.dt, FirstName, LastName, Course_name, Course_description FROM ca_roomlog 
		...
		if ($res_getRL->execute()) {
			...
			if ($res_getRL_rows == 1) {
			$res_getRL->bind_result($roomlogdt,$firstname,$lastname,$coursename,$coursedesc);
			$res_getRL->fetch();
